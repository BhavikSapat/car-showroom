...
export const carService = {
  ...
  async addCar(car: {
    ...
    try {
      const payload = {
        ...
        status: car.status || (car.quantity && car.quantity > 0 ? 'AVAILABLE' : 'OUT_OF_STOCK'),
      };
  ...
  async saveCar(car: { carCompany?: string; carModel?: string; company?: string; model?: string; quantity?: number; [key: string]: any }): Promise<Car> {
    try {
      ...
      return typeof result === 'object' ? result : { id: Date.now(), company: car.company || car.carCompany || '', model: car.model || car.carModel || '' };
    } catch (err: any) {
      return { id: Date.now(), company: car.company || car.carCompany || '', model: car.model || car.carModel || '' };
    }
  },
  ...
  async searchByModel(model: string): Promise<Car[]> {
    try {
      const response = await apiClient.get(`/car/search/model/${encodeURIComponent(model)}`);
  ...
  async searchByCompany(company: string): Promise<Car[]> {
    try {
      const response = await apiClient.get(`/car/search/company/${encodeURIComponent(company)}`);
      return Array.isArray(response.data) ? response.data : [];
    } catch (err: any) {
      return [];
    }
  ...
  async getByStatus(status: string): Promise<Car[]> {
    try {
      const response = await apiClient.get(`/car/status/${encodeURIComponent(status)}`);
      return Array.isArray(response.data) ? response.data : [];
    } catch (err: any) {
      return [];
    }
